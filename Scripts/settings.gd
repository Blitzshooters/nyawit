...
@onready var timer_value:  Label = $HBoxContainer/MarginContainer3/VBoxContainer3/TimerValue
...
func _ready() -> void:
	...
	timer_value.text  = str(int(timer_slider.value)) + "s"
...
func _on_timer_changed(value: float) -> void:
	timer_value.text = str(int(value)) + "s"
	GameSettings.timer_value = value
